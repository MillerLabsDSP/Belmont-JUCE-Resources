...
//==============================================================================
void RMSCompressorAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    rmsCompressor.prepareToPlay(sampleRate);
}
...
void RMSCompressorAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    rmsCompressor.setThreshold(-12.f);
    rmsCompressor.setRatio(4.f);
    rmsCompressor.setKnee(0.f);
    rmsCompressor.setAttack(1.f);
    rmsCompressor.setRelease(1.f);
    
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        auto* channelData = buffer.getWritePointer (channel);
        rmsCompressor.process(channelData, buffer.getNumSamples(), channel);
    }
}
