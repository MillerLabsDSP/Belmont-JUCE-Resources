...
//    
//    int N = buffer.getNumSamples(); // if mono ... possibly 512 or 1024 for stereo
//    
//    auto totalNumInputChannels  = getTotalNumInputChannels();
//    
...
//            buffer.getWritePointer(channel)[n] = gainProcessor(x); // overwrites buffer signal
//        }
//    }
//}
...
void MyFirstPluginAudioProcessor::hardClipProcessor(juce::AudioBuffer<float> & buffer) {
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        for (int n = 0; n < N; n++) {
            ...
            if (x > 0.5f) {
                buffer.getWritePointer(channel)[n] = 0.5f;
            } else if (x < -0.5) {
                buffer.getWritePointer(channel)[n] = -0.5f;
            } else {
